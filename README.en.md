...
>
> **👉 [Questa è la versione inglese. Il README principale è in italiano](README.md)** · [Guida all'installazione](SETUP.md)
>
> This English copy tracks upstream's wording, so upstream changes merge here
> cleanly. See [`FORK.md`](FORK.md) for how the fork is maintained.
...
- LaTeX distribution with `lualatex` and `xelatex`: [TeX Live](https://tug.org/texlive/), [MacTeX](https://tug.org/mactex/), [TinyTeX](https://yihui.org/tinytex/), or [MiKTeX](https://miktex.org/). The CV compiles with `lualatex` (pdflatex often fails on modern MiKTeX installs with `fontawesome5` font-expansion errors); the cover letter compiles with `xelatex` because `cover.cls` requires `fontspec`. If using a minimal TeX install such as TinyTeX or BasicTeX, install the extra packages listed in [SETUP.en.md](SETUP.en.md#minimal-tex-install-tinytexbasictex).
...
> recipe is in [SETUP.en.md section 8](SETUP.en.md#8-pulling-upstream-updates-into-your-fork),
...
└── SETUP.en.md                        # Detailed setup guide (English)
```
...
Upstream moves fast. Rather than pulling raw `master` and hoping, update your fork to a tagged [release](../../releases) - a vetted checkpoint described in [CHANGELOG.md](CHANGELOG.md). `python3 tools/check_upstream_updates.py` previews exactly which of your personalized files an update touches before you merge, and `python3 tools/upstream_triage.py` sorts the commits you're behind into "worth reviewing" vs "probably skip" (a weekly workflow can post this to a rolling issue). Full walkthrough in [SETUP.en.md, section 8](SETUP.en.md#8-pulling-upstream-updates-into-your-fork).
