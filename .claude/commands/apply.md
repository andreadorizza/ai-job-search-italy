...
- Extract: **company name**, **role title**, **department** (if mentioned), **location**, **application deadline** (if the posting states one), and **language** of the posting (Italian or English).
...
1. **Installed portal board:** the host matches any configured job portal in `.agents/skills/` (e.g. `jobindex.dk`, `linkedin.com`, `jobnet.dk`, `jobbank.dk`, `jobdanmark.dk`, `freehire.me`, `europa.eu` (EURES), or any portal added by `/add-portal`).
...
- **Match the language of the job posting** (Italian posting -> Italian cover letter, English posting -> English cover letter). For Italian letters follow `10-mercato-italiano.md`: `Gentile ...` / `Spettabile ...` salutations, `Cordiali saluti,` closing, and a literal Italian date.
