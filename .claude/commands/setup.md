...
- **Job portals:** "The framework ships country-agnostic search CLIs (`linkedin-search`, `freehire-search`, enabled by default) plus Danish portal demos (Jobindex, Jobbank, Jobdanmark, Jobnet) that ship **disabled** and Italian portals for this fork's market. `/scrape` auto-discovers whatever portal skills are installed under `.agents/skills/` and skips any with `enabled: false`. Which portals fit your market?" **Then act on the answer:** if the user's market is Italy (the default for this fork), enable the Italian portal skills; if their market is Denmark (or they ask for the Danish boards), edit each of the four Danish `SKILL.md` files and set `enabled: true` in the frontmatter; otherwise leave them disabled and say so. **Never enable a skill whose frontmatter says `access: restricted`** - those are gated behind `AI_JOB_SEARCH_ALLOW_RESTRICTED` on purpose (see `FORK.md`); mention they exist and let the user opt in themselves - they cost nothing while disabled and can be enabled later by flipping the flag. If the user needs a local board that is not shipped, guide them to `/add-portal` (market-specific skills live in their fork). WebSearch/`site:` queries remain the fallback for portals without a CLI skill.
- **CV language:** "Should your CVs be written in Italian (the default for this fork), or in English (widely accepted at multinationals and in tech)?" Record the answer as a `CV language: <language>` line in CLAUDE.md's Identity section. Cover letters always match each posting's language automatically; this setting governs the CV only. If the user wants both, record `CV language: match the posting - Italiano for Italian-language postings, English for all others` - `/apply` honours that form. If the user is unsure, keep Italian and note they can re-run `/setup --section search` to change it.
