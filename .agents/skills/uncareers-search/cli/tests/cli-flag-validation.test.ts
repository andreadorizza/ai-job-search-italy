...
describe("unknown flags are rejected, never silently dropped", () => {
  ...
  test("detail rejects a search-only flag", async () => {
    const r = await runCLI(["detail", "285103", "--home-based"])
    expect(r.exitCode).toBe(1)
    expect(errorOf(r.stderr).code).toBe("UNKNOWN_FLAG")
  })
})
...
describe("required arguments", () => {
  ...
  for (const flag of ["-q", "-l", "--category", "--network"]) {
    test(`${flag} given without a value fails instead of dropping the filter`, async () => {
  ...
  test("-l with only separators fails", async () => {
    const r = await runCLI(["search", "-l", ";,"])
    expect(r.exitCode).toBe(1)
    expect(errorOf(r.stderr).code).toBe("MISSING_REQUIRED")
  })
})
...
describe("flag values are validated", () => {
  ...
  test("--home-based does not swallow a stray word", async () => {
    const r = await runCLI(["search", "--home-based", "engineer"])
    expect(r.exitCode).toBe(1)
    expect(errorOf(r.stderr).code).toBe("BAD_ARG")
    expect(errorOf(r.stderr).error).toContain("--query")
  })

  test("--home-based rejects a non-boolean value", async () => {
    const r = await runCLI(["search", "--home-based=yes"])
    expect(r.exitCode).toBe(1)
  })

  test("detail --format table is not a detail format", async () => {
    const r = await runCLI(["detail", "285103", "--format", "table"])
    expect(r.exitCode).toBe(1)
    expect(errorOf(r.stderr).code).toBe("BAD_ARG")
  })
})
...
describe("detail input validation happens before any request", () => {
  test("a URL on another host fails with BAD_ID", async () => {
    const r = await runCLI(["detail", "https://evil.example/jobSearchDescription/1"])
    expect(r.exitCode).toBe(1)
    expect(errorOf(r.stderr).code).toBe("BAD_ID")
  })

  test("a non-numeric id fails with BAD_ID", async () => {
    const r = await runCLI(["detail", "285103 OR 1"])
    expect(r.exitCode).toBe(1)
    expect(errorOf(r.stderr).code).toBe("BAD_ID")
  })
})
